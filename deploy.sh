...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-20002-infrastructure \
     --values test-20002-infrastructure/values.yaml \
     --values test-20002-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/jaebchoi/test-20002 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "test-20002 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-20002 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace test-20002 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/jaebchoi/test-20002 \
      --path test-20002-deploy/src/main/resources \
      --revision $branch \
...
down() {
  if is_app_running; then
    ...
    echo "test-20002 is not deployed"
  fi
}
